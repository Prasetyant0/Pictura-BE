                    ...
                    <div class="mb-3">
                        ...
                        <div class="flex gap-2">
                            <select id="albums_id"
                                class="bg-gray-50 border border-gray-300 text-colorText text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5"
                                name="albums_id">
                                <option selected value="">Select albums or add new albums</option>
                                @foreach ($albums as $a)
                                    <option value="{{ $a->id }}">{{ $a->album_name }}</option>
                                @endforeach
                            </select>
                            <button type="button" class="bg-btnsec w-10 p-2 rounded-lg" data-modal-toggle="add-new-albums"
                                data-modal-target="add-new-albums"><i class="bi bi-plus-lg"></i></button>
                        </div>
                    </div>
...
@push('jsInternalAuth')
    ...
    <script>
        $(document).ready(function() {
            $('#albumFormAdd').submit(function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#albums_id').append('<option value="' + response.album.id + '">' +
                            response.album.album_name + '</option>');
